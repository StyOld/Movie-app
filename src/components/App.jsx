...
// вынес в другой файлп подклбчение иконок
import "./fortawesome";
...
class App extends React.Component {
  componentDidMount() {
    ...
    if (session_id) {
      // вынести в actionCreator "GET_ACCOUNT"
      CallApi.get("/account", {
        params: {
          session_id
        }
  ...
  // вынести в middleware на занятии показывал
  ...
  render() {
    ...
    return isAuth || !session_id ? (
      <BrowserRouter>
        <div>
          {/*
            Header сделать умным компонентом (с connect)
          */}
          <Header user={user} />
          ...
          <Route
            ...
            component={AccountListByTypePage("favorite")}
          />
          <Route
            path="/account/watchlist"
            component={AccountListByTypePage("watchlist")}
          />
        </div>
      </BrowserRouter>
    ) : (
